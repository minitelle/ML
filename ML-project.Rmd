...
#BeginTS <- Sys.time();
#
modelTrain <- train(classe ~., method="rf", data=trainingSet, trControl=trainControl(method="cv"),number = 5)
#
#EndTS <- Sys.time();
...
We can see that applying the prediction model on the validation data set a `r CFtrain$overall[1]` accuracy with a tiny out of sample error `r OOSE`, which allows us to be pretty confident in this model.
...
str(testQuiz)
## Accuracy
#CFtest <- confusionMatrix(predTest, testQuiz[,-length(names(testQuiz))]$classe)
#CFtest$overall[1]
#OOSE <- 1 - as.numeric(CFtest$overall[1])
#OOSE
```
\newpage
